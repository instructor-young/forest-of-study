...
function StudyCard({ study }) {
  ...
  return (
    ...
    >
      {imgClassName && <img className={cx("rounded-[20px] absolute h-full w-full bg-cover", imgClassName)} />}

      <article className={cx("p-[30px] rounded-[20px] h-full w-full", colorClassName, "group-data-[theme=dark]:backdrop-brightness-[45%]")}>
        <header>
          <div className="flex items-center justify-between">
            <h6 className="text-lg font-bold">
              <span className={cx("mr-0.5", ownerNameClassName)}>{study.ownerName}</span>
              <span className={cx(theme === "dark" ? "text-white" : "text-black-414141")}>의 {study.name}</span>
            </h6>

            <Tag theme={theme} type={"point"} value={study.point} />
          </div>
